...
func _spawn_decorations():
	...
	_place(KHAEN, Vector3(LEFT_SIDE_X, 0.0, randf_range(-3.0, 3.0)), Vector3(0.2, 0.2, 0.2), false)
	_place(KHAEN, Vector3(RIGHT_SIDE_X, 0.0, randf_range(-3.0, 3.0)), Vector3(0.2, 0.2, 0.2), false)
	...
	if dist_since_gate >= 300.0:
		...
		if randf() < chance:
			_place(CITYGATE, Vector3(0.0, 0.0, 0.0), Vector3(9.0, 9.0, 9.0), false)
	...
	if not gate_placed_this_tile and randf() < 0.4:
		...
		_place(SIRINDHORNAE, Vector3(side_x, 0.0, randf_range(-3.0, 3.0)), Vector3(1.0, 1.0, 1.0), true)

	# 4. Kinareemimus — random on each side independently
	if randf() < 0.5:
		var s = randf_range(0.2, 0.3)
		_place(KINARE, Vector3(LEFT_SIDE_X + randf_range(-2.0, 2.0), 0.0, randf_range(-4.0, 4.0)), Vector3(s, s, s), true)
	if randf() < 0.5:
		var s = randf_range(0.2, 0.3)
		_place(KINARE, Vector3(RIGHT_SIDE_X + randf_range(-2.0, 2.0), 0.0, randf_range(-4.0, 4.0)), Vector3(s, s, s), true)
	...
	for _i in range(randi_range(1, 3)):
		...
		_place(TREEV1, Vector3(side_x, 0.0, randf_range(-4.0, 4.0)), Vector3(1500.0, 1500.0, 1500.0), true)
