...
func _setup_modern_hud():
	...
	hud_panel.custom_minimum_size = Vector2(250, 160) # Increased height for buttons
	hud_panel.mouse_filter = Control.MOUSE_FILTER_PASS # Allow clicks to pass through to buttons
	add_child(hud_panel)
...
func _update_button_visuals():
	...
	if player.charges >= player.MAX_CHARGES and !is_cooldown:
		...
		if player.is_rolling_skill:
			...
			skill_button.disabled = true # Disable while rolling
